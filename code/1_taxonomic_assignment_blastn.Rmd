---
title: "Taxonomic Assignment of NBS 2023 Mifish Amplicons"
...
- this is the fasta seqs from the 20250129 run on dadasnake of just the NBS23 samples, and blastn using the ncbi nt database with 96% seq identity and 98% query coverage   

```{r load-output-from-taxonkit}
taxonomy <-read.delim("/home/kimberly.ledger/NBS23_mifish_mb/data/blastn_taxlineage.txt", header = FALSE, na.strings=c(""," ","NA"))
head(taxonomy)
```
...
this particular project doesn't look at non-fish so will remove them all here
```{r}
taxon_df <- taxon_df %>%
  filter(class == "Actinopteri")
```
...
```{r}
spp_range <- spp_range %>%
  mutate(in_range = ifelse(Species == "natural gynogenetic Carassius auratus red var.", "no", in_range)) %>%
...
```{r}
write.csv(tax_df_2, "/home/kimberly.ledger/NBS23_mifish_mb/outputs/taxonomy_20250129_uncollapsed.csv")
write.csv(tax_df_3, "/home/kimberly.ledger/NBS23_mifish_mb//outputs/taxonomy_20250129_collapsed.csv")
```